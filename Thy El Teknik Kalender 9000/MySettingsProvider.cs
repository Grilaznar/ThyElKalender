  ...
  class MySettingsProvider : SettingsProvider
  {
    ...
    public override SettingsPropertyValueCollection GetPropertyValues(SettingsContext context, SettingsPropertyCollection collection)
    {
      ...
      foreach (SettingsProperty setting in collection)
      {
        ...
        var t = setting.PropertyType;

        if (SettingsDictionary.ContainsKey(setting.Name))
        {
          value.SerializedValue = SettingsDictionary[setting.Name].value;
          value.PropertyValue = 
            TypeDescriptor.GetConverter(t).ConvertFromInvariantString(SettingsDictionary[setting.Name].value); //Convert.ChangeType(SettingsDictionary[setting.Name].value, t);
        }
        else //use defaults in the case where there are no settings yet
        {
          value.SerializedValue = setting.DefaultValue;
          value.PropertyValue =
            TypeDescriptor.GetConverter(t).ConvertFromInvariantString((string)setting.DefaultValue);// Convert.ChangeType(setting.DefaultValue, t);
        }
